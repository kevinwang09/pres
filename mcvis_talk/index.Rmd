...
+ The aim of a batsman is to score as many **runs** as possible bfore getting **out**.
...
$$\text{batting ave} = \frac{\text{runs}}{\text{no. of outs}}, \qquad \text{or equvalently, } \qquad \texttt{log_runs} = \texttt{log_ave} + \texttt{log_outs}.$$
...
MC occurs when columns of $X$ are almost linear dependent. 
That is, if any column of $X$ can be expressed as a linear combination of other columns. 
...
```{r, echo = FALSE}
M2 = lm(log_hs ~ . -log_ave - log_outs - log_runs, data = X)
M3 = lm(log_hs ~ . -log_runs, data = X)
...
sjPlot::tab_model(M1, M2, M3,
                  ...
                  show.r2 = FALSE, 
                  show.obs = FALSE,
                  dv.labels = c("Include all", "Remove all", "Remove log_runs"))
```
...
+ Identifying high correlation does not always identify sources of MC.
...
2. Eigenvalues of $X^\top X$ (the "uncentered covariance matrix"), $\lambda_{1}\geq\lambda_{2}\geq{\ldots}\geq{\lambda_{p}}\geq 0$. 

A **smaller** value of $\lambda_{p}$ produces a matrix determinant closer to 0, which implies linear dependence and thus MC (Stewart 1987).  
...
where $\tau_{j}=1/\lambda_{j}, \quad j=1,\ldots,p$. So that a larger $\tau_p$ value implies great MC.  

--

+ This tells us how these diagnostic measures are related.

+ But it does **not** tell us about which variable $\boldsymbol{x}_j$ causes the increase of $\tau_p$, which is an indicator of MC.

--

.blockquote[
By understanding the linearity between $\tau_p$ and each VIFs (which is an linear regression problem in itself), we can get a better diagnosis of which variables are causing MC (complex).
]
